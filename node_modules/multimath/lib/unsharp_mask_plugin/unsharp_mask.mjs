...
import hsl_l16 from './hsl_l16.mjs'
...
export default function unsharp (img, width, height, amount, radius, threshold) {
  ...
  const lightness = hsl_l16(img, width, height)

  const blured = new Uint16Array(lightness) // copy, because blur modify src

  blurMono16(blured, width, height, radius)

  const amountFp = (amount / 100 * 0x1000 + 0.5)|0
  const thresholdFp = (threshold * 257)|0
    ...
    if (Math.abs(diff) >= thresholdFp) {
      ...
      if (s === 0) {
        ...
        r = (hShifted >= 0xaaaa)
          ...
          : (hShifted >= 0x7fff)
              ? m1 + ((m2 - m1) * 6 * (0xaaaa - hShifted) + 0x8000 >> 16)
              : (hShifted >= 0x2aaa)
                  ? m2 // 0x2aaa == 0xffff / 6
